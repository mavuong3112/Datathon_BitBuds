...
print(f"{elapsed()} Loading Stage 8 _train data …  (BUILD_MODE={BUILD_MODE})")
# Load only the branch we need to fit in 32GB RAM — both branches would exceed limits.
# pos_test is needed for label construction (val period positives), so always load.
...
if BUILD_MODE in ('train','both'):
    ...
    # Stage 15 memory-budget: drop cands_train rows for users with NO pos_test activity
    # (these are guaranteed label=0 → only negative noise + memory waste).
    # pos_test = full positive events; users not in pos_test will all be label=0 in train.
    _pos_test_users = set(pd.read_parquet(f"{CACHE_DIR}/user_item_pos.parquet", columns=['user_id'])['user_id'].unique())
    _n_before = len(cands_train)
    cands_train = cands_train[cands_train['user_id'].isin(_pos_test_users)].reset_index(drop=True)
    print(f"{elapsed()}   pruned cands_train to users-with-labels: {_n_before:,} → {len(cands_train):,}")
    del _pos_test_users
    ...
    print(f"{elapsed()}   train: cands={len(cands_train):,} pos={len(pos_train):,} inter={len(inter_train):,}")
if BUILD_MODE in ('test','both'):
    ...
    print(f"{elapsed()}   test:  cands={len(cands_test):,} pos={len(pos_test):,} inter={len(inter_test):,}")
# For BUILD_MODE='train', pos_test is also needed for label construction.
# For BUILD_MODE='test', pos_train is not needed at all (still need it for build_pos_feat? No — only pos_test used for test branch).
...
# Stage 15: Hook #2 (item_quality_score, price_diff_vs_peer) REMOVED
# Reason: not in top-15 LGBM/XGB FI in Stage 14a (LB 0.2438 vs Stage 12 0.2440 = -0.0002).
# Decision: drop both to reduce feature dimensionality + noise.
...
# Stage 15: Hook #3 Part A REMOVED (is_fresh_1d corr=-0.017 in train, hurt LB in Stage 14a)
# Replaced by snapshot 24h velocity features (real fresh signal from fact_listing_snapshot)
snap_feat = pd.read_parquet(f"{CACHE_DIR}/snapshot_features.parquet",
                             columns=['item_id','views_24h_log','contacts_24h_log','contact_rate_24h'])
snap_feat = snap_feat.rename(columns={
    'views_24h_log':    'snap_views_24h_log',
    'contacts_24h_log': 'snap_contacts_24h_log',
    'contact_rate_24h': 'snap_contact_rate_24h',
})
items_feat = items_feat.merge(snap_feat, on='item_id', how='left')
for _col in ['snap_views_24h_log','snap_contacts_24h_log','snap_contact_rate_24h']:
    items_feat[_col] = items_feat[_col].fillna(0).astype(np.float32)
del snap_feat; gc.collect()
print(f"{elapsed()}   snapshot 24h merged: views_24h_log mean={items_feat['snap_views_24h_log'].mean():.3f}, "
      f"contacts_24h_log mean={items_feat['snap_contacts_24h_log'].mean():.3f}")
...
ITEM_COLS = [
    ...
    # Stage 15: snapshot 24h velocity (real fresh signal from fact_listing_snapshot)
    'snap_views_24h_log','snap_contacts_24h_log','snap_contact_rate_24h',
]
...
# ── Stage 14 Hook #1: District transition matrix (load + flatten) ─────────────
import pickle
_dt_pkl_path = f"{CACHE_DIR}/district_transition.pkl"
if os.path.exists(_dt_pkl_path):
    with open(_dt_pkl_path, 'rb') as _f:
        _dt_dict = pickle.load(_f)
    _dt_rows = [(src, tgt, prob) for src, tgts in _dt_dict.items() for tgt, prob in tgts.items()]
    district_trans_df = pd.DataFrame(_dt_rows,
                                      columns=['pref_district_name','district_name','district_transition_prob'])
    district_trans_df['district_transition_prob'] = district_trans_df['district_transition_prob'].astype(np.float32)
    print(f"{elapsed()}   district_transition.pkl loaded: {len(_dt_dict):,} sources, {len(district_trans_df):,} pairs")
    del _dt_dict, _dt_rows
else:
    print(f"{elapsed()}   WARNING: {_dt_pkl_path} not found — district_transition_score will be 0")
    district_trans_df = pd.DataFrame(columns=['pref_district_name','district_name','district_transition_prob'])
...
inter_feat_train = inter_feat_test = None
if 'inter_train' in globals():
    inter_feat_train = build_inter_feat(inter_train); del inter_train; gc.collect()
if 'inter_test' in globals():
    inter_feat_test  = build_inter_feat(inter_test);  del inter_test; gc.collect()
print(f"{elapsed()}   inter_feat: train={'-' if inter_feat_train is None else f'{len(inter_feat_train):,}'} "
      f"test={'-' if inter_feat_test is None else f'{len(inter_feat_test):,}'}")

# ── Pos features (TWO versions) ───────────────────────────────────────────────
print(f"{elapsed()} Building pos_feat (1-2 versions) …")
if 'pos_train' in globals():
...
pos_feat_train = build_pos_feat(pos_train, val_split_dt) if 'pos_train' in globals() else None
pos_feat_test  = build_pos_feat(pos_test,  train_end_dt)  if BUILD_MODE in ('test','both') else None
print(f"{elapsed()}   pos_feat: train={'-' if pos_feat_train is None else f'{len(pos_feat_train):,}'} "
      f"test={'-' if pos_feat_test is None else f'{len(pos_feat_test):,}'}")
...
# Free MODE-irrelevant data BEFORE hist build to avoid python+DuckDB RAM contention
def _safe_del(names):
    for _n in names:
        if _n in globals():
            del globals()[_n]
if BUILD_MODE == 'test':
    _safe_del(['cands_train','pos_feat_train','pos_train','inter_feat_train'])
    gc.collect()
    print(f"{elapsed()}   [BUILD_MODE=test] freed _train branches before hist_decay")
elif BUILD_MODE == 'train':
    # NOTE: pos_test is retained — needed later for val-label construction.
    _safe_del(['cands_test','pos_feat_test','inter_feat_test'])
    gc.collect()
    print(f"{elapsed()}   [BUILD_MODE=train] freed _test branches before hist_decay")

print(f"{elapsed()} Building hist_decay (1-2 versions) via DuckDB …")
import duckdb
...
hist_train = None; hist_test = None
if BUILD_MODE in ('train','both'):
...
FEATURE_COLS = (
    ...
    # Stage 14 Hook #1: district transition (probability-soft adjacency)
    ['district_transition_score']
)
...
def build_features(cands_df, pos_feat_df, hist_df, inter_feat_df,
    ...
    # Stage 14 Hook #1: district_transition_score (probability-soft adjacency)
    feat = feat.merge(district_trans_df, on=['pref_district_name','district_name'], how='left')
    feat['district_transition_prob'] = feat['district_transition_prob'].fillna(0).astype(np.float32)
    # If exact match → 1.0 (override); else use observed transition probability
    feat['district_transition_score'] = np.where(
        feat['district_match'] == 1,
        np.float32(1.0),
        feat['district_transition_prob']
    ).astype(np.float32)
    feat = feat.drop(columns=['district_transition_prob',
                              'price_bucket','ad_type','district_name','seller_type',
...
del _pos_val_mask; gc.collect()
# pos_train/pos_test may have been freed earlier in BUILD_MODE-aware hist prep
for _v in ('pos_train','pos_test'):
    if _v in globals():
        del globals()[_v]
gc.collect()

# Stage 9: free unused branches BEFORE big merge (may be already freed for hist)
if BUILD_MODE == 'test':
    _free = ('cands_train','pos_feat_train','hist_train','inter_train','inter_feat_train')
elif BUILD_MODE == 'train':
    _free = ('cands_test','pos_feat_test','hist_test','inter_test','inter_feat_test')
else:
    _free = ()
for _v in _free:
    if _v in globals():

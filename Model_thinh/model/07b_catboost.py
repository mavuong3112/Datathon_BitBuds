...
# ── Train CatBoost YetiRank — Stage 16: 5-seed bagging for variance reduction ─
print(f"{elapsed()} Training CatBoost YetiRank (5-seed bag) …")
...
SEEDS = [int(s) for s in os.environ.get('CB_SEEDS', '42,123,456,789,2024').split(',')]
...
for si, seed in enumerate(SEEDS):
    ...
    m = cb.CatBoostRanker(
        ...
        od_wait=50,
        random_seed=seed,
        verbose=100 if si == 0 else 0,
    )
    m.fit(trn_pool, eval_set=val_pool)
    print(f"{elapsed()}     best_iter={m.best_iteration_}")
    models.append(m)

# Save last-seed model + FI (representative)
models[-1].save_model(f"{CACHE_DIR}/catboost_ranker.cbm")
fi = pd.Series(models[-1].feature_importances_, index=FEATURE_COLS).sort_values(ascending=False)
print(f"\n{elapsed()} CatBoost Top 15 FI (last seed):")
...
print(f"{elapsed()} CatBoost predicting (averaging {len(models)} seeds) …")
all_scores = np.zeros(len(X_test), dtype=np.float64)
for si, m in enumerate(models):
    s = m.predict(X_test)
    all_scores += s
    print(f"{elapsed()}   seed {si+1}/{len(models)}: mean={s.mean():.4f} std={s.std():.4f}")
cat_scores = (all_scores / len(models)).astype(np.float32)
print(f"{elapsed()} Bagged predictions: {cat_scores.shape}  mean={cat_scores.mean():.4f}  std={cat_scores.std():.4f}")

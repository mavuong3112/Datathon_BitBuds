...
EASE: skipped (n_items=691K → 1.9TB matrix, not feasible).

Outputs:
  cache/als_candidates.parquet   — BPR top-N per user
  cache/ease_candidates.parquet  — ALS top-N per user (named for compatibility)
...
# ── Load interactions ─────────────────────────────────────────────────────────
print(f"{elapsed()} Loading interactions …")
...
with open(f"{CACHE_DIR}/mappings.pkl", 'wb') as f:
...
BPR_U_PATH = f"{CACHE_DIR}/als_model/user_factors_bpr.npy"
BPR_I_PATH = f"{CACHE_DIR}/als_model/item_factors_bpr.npy"
...
BPR_CANDS_PATH = f"{CACHE_DIR}/als_candidates.parquet"
if os.path.exists(BPR_CANDS_PATH):
    print(f"{elapsed()} [SKIP] BPR candidates already exist — loading factors for ALS step")
    user_factors_bpr = np.load(f"{CACHE_DIR}/als_model/user_factors_bpr.npy")
    item_factors_bpr = np.load(f"{CACHE_DIR}/als_model/item_factors_bpr.npy")
    ...
    rows_bpr = []
    CHUNK = 1000   # users per chunk for numpy matmul
...
ALS_CANDS_PATH = f"{CACHE_DIR}/ease_candidates.parquet"
if os.path.exists(ALS_CANDS_PATH):
    ...
    try:
        ...
        if os.path.exists(ALS_MODEL_U) and os.path.exists(ALS_MODEL_I):
            ...
            u_factors_als = als.user_factors
            i_factors_als = als.item_factors

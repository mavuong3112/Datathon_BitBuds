...
# Replace inf → NaN: log1p of bad inputs can sink → inf, which crashes LightGBM
# (NaN is fine — LightGBM has native NaN branch)
print(f"{elapsed()} Converting to float32 + replacing inf → NaN …")
...
print(f"{elapsed()} Loading test features (+ inf→NaN sanitize) …")
for _col in FEATURE_COLS:
...
test_blend = test_df['blend_score'].astype(np.float32).values  # for step 8 tiebreaker

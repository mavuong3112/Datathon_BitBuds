...
params = {
    ...
    'eval_metric': 'ndcg@10',
    'eta': 0.05,
    'max_depth': 6,         # Stage 14: 7→6 to reduce histogram memory
    'min_child_weight': 1,
    'subsample': 0.5,       # Stage 14: 0.8→0.5 (24M→12M rows per tree fits RAM)
    'colsample_bytree': 0.8,
    'tree_method': 'hist',
    'max_bin': 128,         # Stage 14: 256→128 to halve histogram size
    # Stage 14: 76 features pushed GPU over 8GB limit — train on CPU (slower but stable).
...
evals_result = {}
model = xgb.train(params, dtrn,
                  num_boost_round=500,
                  evals=[(dval, 'val')],
                  evals_result=evals_result,
                  early_stopping_rounds=50,
                  verbose_eval=50)
print(f"{elapsed()} Best iteration: {model.best_iteration}")

model.save_model(f"{CACHE_DIR}/xgboost_ranker.json")

# Feature importance
fi = pd.Series(model.get_score(importance_type='gain'), name='gain').sort_values(ascending=False)
print(f"\n{elapsed()} XGBoost Top 15 FI:")
...
print(f"{elapsed()} XGBoost predicting (chunked, GPU OOM avoidance) …")
# Switch model to CPU for prediction to avoid OOM
...
print(f"{elapsed()} Predictions: {xgb_scores.shape}  mean={xgb_scores.mean():.4f}  std={xgb_scores.std():.4f}")

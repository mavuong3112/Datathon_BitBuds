...
OUT_PATH       = f"{CACHE_DIR}/sasrec_candidates{SUFFIX}.parquet"
MODEL_PATH     = f"{CACHE_DIR}/sasrec_model{SUFFIX}.pt"
...
# Stage 8: in train mode, filter sequences to events < VAL_SPLIT + exclude Tết
if MODE == 'train':
    seq['event_ts'] = pd.to_datetime(seq['event_ts'])
    val_split_dt = pd.Timestamp(VAL_SPLIT)
    tet_start = pd.Timestamp('2026-02-14'); tet_end = pd.Timestamp('2026-02-23')
    before = len(seq)
    seq = seq[(seq['event_ts'] < val_split_dt) &
              ~((seq['event_ts'] >= tet_start) & (seq['event_ts'] < tet_end))]
    print(f"{elapsed()} seq train-mode filter: {before:,} → {len(seq):,} ({100*len(seq)/before:.1f}% retained)")
...
df_sr.to_parquet(OUT_PATH, index=False)

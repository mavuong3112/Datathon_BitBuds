...
# Stage 9: BLEND_WEIGHT controls LightGBM vs CatBoost blend for warm users
#  - 1.0 = LightGBM only (Stage 8)
#  - 0.0 = CatBoost only
#  - 0.6 = recommended 0.6×LGBM + 0.4×CatBoost
BLEND_WEIGHT = float(os.environ.get('BLEND_WEIGHT', '1.0'))
assert 0.0 <= BLEND_WEIGHT <= 1.0

print(f"COLD_STRATEGY = {COLD_STRATEGY}  COLD_MODE = {COLD_MODE}  BLEND_WEIGHT = {BLEND_WEIGHT}")
...
# Stage 9: load CatBoost predictions if BLEND_WEIGHT < 1.0
if BLEND_WEIGHT < 1.0:
    ...
    if os.path.exists(cb_path):
        ...
        # Merge into ranked dataframe — should align with same (user_id, item_id) pairs
        # ranked has top-30 per user only; cb has ALL test rows. Use merge.
        ranked = ranked.merge(cb[['user_id','item_id','catboost_score']],
                                on=['user_id','item_id'], how='left')
        # Per-user min-max normalize both scores, then blend
        ...
        ranked['lgbm_score']    = BLEND_WEIGHT * ranked['lgbm_norm'] + (1 - BLEND_WEIGHT) * ranked['catboost_norm']
        print(f"{elapsed()} Blended: {BLEND_WEIGHT}×LGBM + {1-BLEND_WEIGHT}×CatBoost")
        ranked = ranked.drop(columns=['lgbm_norm','catboost_norm','catboost_score'])
        del cb
    else:
        print(f"{elapsed()} CatBoost predictions not found, BLEND_WEIGHT={BLEND_WEIGHT} ignored")
